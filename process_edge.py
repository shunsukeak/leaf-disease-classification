# This is for device-side processing
import os
import pandas as pd
...
##for test data size calcuration

# test_dir = "./test_images_from_edge"
...
#     dest_path = os.path.join(test_dir, os.path.basename(source_path))
#     shutil.copy(source_path, dest_path)

# print(f"Test images saved to {test_dir}")
# print(f"Total test image size: {total_size / (1024 * 1024):.2f} MB")
...
## for getting apply heatmap images!
...
#     blended = cv2.addWeighted(image_array, 0.7, heatmap, 0.3, 0)
    
#     return Image.fromarray(blended)

## for getting white-black masking
# def apply_mask(image, mask):
#     if isinstance(mask, torch.Tensor):
#         mask = mask.cpu().numpy()
...
#     result = image_array * binary_mask + bw_image * (1 - binary_mask)
#     result = result.astype(np.uint8)
    
#     return Image.fromarray(result)

# for applying black mask 
def apply_mask(image, mask):
            ...
            for idx, pred in enumerate(predicted):
                if pred.item() == 1:
                    ...
                    if global_idx < len(test_loader.dataset):
                        ...
                        # image process for Grad-CAM
                        with torch.enable_grad():
                            image = images[idx].unsqueeze(0).requires_grad_(True)
                            mask = grad_cam.generate_heatmap(image)
                        
                        original_image = Image.open(original_image_path).convert('RGB')
                        processed_image = apply_mask(original_image, mask)
                        
                        save_dir = os.path.join('processed_test_0.95', disease_name)
                        os.makedirs(save_dir, exist_ok=True)
                        save_path = os.path.join(save_dir, f"image_{global_idx}_gradcam.jpg")
